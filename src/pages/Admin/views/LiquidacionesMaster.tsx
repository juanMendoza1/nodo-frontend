...
import { 
  ...
  Settings2, Activity, Trash2 // 🔥 IMPORTAMOS Trash2
} from 'lucide-react';
...
import ConfirmDialog from '../../../components/ui/ConfirmDialog'; // 🔥 ASEGÚRATE DE TENER EL DIALOG IMPORTADO
...
export default function LiquidacionesMaster() {
  ...
  const [nuevaLiquidacion, setNuevaLiquidacion] = useState({ codigo: '', nombre: '' });
  ...
  // 🔥 ESTADO PARA CONFIRMAR ELIMINACIÓN DE LIQUIDACIÓN
  const [confirmDialog, setConfirmDialog] = useState({ isOpen: false, id: 0, nombre: '' });

  const EMPRESA_MASTER_ID = 1; 

  useEffect(() => {
    programasService.obtenerTodos().then(data => {
      setProgramas(data || []);
      if (data && data.length > 0) setProgramaSeleccionado(data[0].id);
    });
  }, []);

  useEffect(() => {
    if (programaSeleccionado) cargarLiquidaciones(programaSeleccionado);
  ...
  const handleCrearLiquidacion = async (e: React.FormEvent) => {
    ...
    toast.promise(promesa, {
      ...
      success: () => {
        ...
        setNuevaLiquidacion({ codigo: '', nombre: '' });
        cargarLiquidaciones(programaSeleccionado!);
        return 'Liquidación creada con éxito';
      },
  ...
  const confirmarEliminacion = async () => {
    toast.promise(
      liquidacionesService.eliminarPlantilla(confirmDialog.id),
      {
        ...
        error: (err) => {
          setConfirmDialog({ isOpen: false, id: 0, nombre: '' });
          // Muestra exactamente el mensaje que armamos en el Backend
  ...
  const guardarRelacion = async () => {
    ...
    toast.promise(liquidacionesService.configurarReceta(EMPRESA_MASTER_ID, payload), {
      loading: 'Guardando relación...',
      success: () => {
        setIsBuilderOpen(false);
        return 'Conceptos relacionados correctamente';
      },
    ...
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 relative h-full flex flex-col">
      ...
      <div className="flex justify-between items-end mb-6">
        <div>
          <h2 className="text-3xl font-black text-zinc-900 flex items-center gap-3 tracking-tight">
            <Calculator className="w-8 h-8 text-black" /> Parametrizar Liquidaciones
          </h2>
          <p className="text-sm text-zinc-500 font-medium mt-1">
            Gestiona qué conceptos se resuelven en cada tipo de liquidación.
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-6 flex-1 min-h-0">
        ...
        <div className="w-full lg:w-64 shrink-0 bg-white border border-zinc-200 rounded-3xl p-4 shadow-sm flex flex-col">
          <p className="text-[10px] font-extrabold uppercase tracking-widest text-zinc-400 mb-3 px-2">Módulos de Software</p>
        ...
        <div className="flex-1 bg-white border border-zinc-200 rounded-3xl shadow-sm flex flex-col overflow-hidden">
          <div className="p-5 border-b border-zinc-100 flex justify-between items-center bg-zinc-50/50">
            <h3 className="font-extrabold text-zinc-900 flex items-center gap-2 text-lg">
              <Layers className="w-5 h-5 text-zinc-400" /> Liquidaciones Configuradas
            </h3>
            <button onClick={() => setIsPlantillaModalOpen(true)} disabled={!programaSeleccionado} className="bg-black text-white px-4 py-2 rounded-xl text-sm font-bold flex items-center gap-2 hover:bg-zinc-800 transition-all shadow-md disabled:opacity-50">
          ...
          <div className="flex-1 overflow-auto p-6 bg-zinc-50/30">
            {loading ? (
              ...
              <div className="text-center py-20 text-zinc-500 font-medium">No hay liquidaciones configuradas para este módulo.</div>
            ) : (
                  ...
                  <div key={liq.id} className="bg-white border border-zinc-200 rounded-2xl p-5 hover:shadow-md hover:border-black transition-all duration-300 group relative flex flex-col justify-between">
                    <div className="flex justify-between items-start mb-3">
                      <div>
                        ...
                        <span className="bg-zinc-100 text-zinc-500 px-2 py-1 mt-2 inline-block rounded text-[10px] font-extrabold uppercase tracking-widest">{liq.codigo}</span>
                      </div>
                      
                      {/* 🔥 BOTÓN DE ELIMINAR (Solo visible al hacer hover) */}
                      <button 
                        onClick={() => setConfirmDialog({ isOpen: true, id: liq.id, nombre: liq.nombre })} 
                    ...
                    <div className="pt-3 border-t border-zinc-100 flex justify-end">
                       <button onClick={() => abrirRelacionador(liq)} className="text-xs font-bold text-indigo-600 hover:text-indigo-800 flex items-center gap-1 opacity-90 group-hover:opacity-100 transition-opacity">
                         <Settings2 className="w-3.5 h-3.5" /> Relacionar Conceptos <ChevronRight className="w-3.5 h-3.5" />
                       </button>
                    </div>
                  </div>
                ))}
      ...
      {isPlantillaModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-zinc-900/60 backdrop-blur-sm px-4">
          <div className="bg-white rounded-3xl w-full max-w-sm p-6 shadow-2xl">
             <h3 className="font-black text-lg mb-5 text-zinc-900">Nueva Liquidación</h3>
             <form onSubmit={handleCrearLiquidacion} className="space-y-4">
               <div>
                 ...
                 <input required value={nuevaLiquidacion.codigo} onChange={e => setNuevaLiquidacion({...nuevaLiquidacion, codigo: e.target.value.toUpperCase().replace(/\s/g, '_')})} className="w-full bg-zinc-50 border border-zinc-200 rounded-xl px-3 py-2.5 text-sm font-mono font-bold focus:bg-white focus:border-black outline-none uppercase transition-all" placeholder="Ej. LIQ_NOMINA" />
               </div>
               <div>
                 <label className="text-[10px] font-extrabold uppercase tracking-widest text-zinc-500 mb-1.5 block">Nombre Descriptivo</label>
                 <input required value={nuevaLiquidacion.nombre} onChange={e => setNuevaLiquidacion({...nuevaLiquidacion, nombre: e.target.value})} className="w-full bg-zinc-50 border border-zinc-200 rounded-xl px-3 py-2.5 text-sm font-bold focus:bg-white focus:border-black outline-none transition-all" placeholder="Ej. Nómina Operarios" />
               </div>
      ...
      <div className={`fixed top-0 right-0 h-full w-full max-w-3xl bg-white shadow-2xl z-50 transform transition-transform duration-300 flex flex-col ${isBuilderOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="flex items-center justify-between p-5 border-b border-zinc-100 bg-zinc-50/50 shrink-0">
          <div>
             <h3 className="text-lg font-black text-zinc-900 flex items-center gap-2">
               <Activity className="w-4 h-4 text-indigo-600"/> Relacionar Conceptos
             </h3>
             <p className="text-[11px] text-zinc-500 font-medium mt-0.5">
               Configurando: <span className="font-bold text-zinc-800 uppercase">{liquidacionActiva?.nombre}</span>
             </p>
          </div>
        ...
        <div className="flex-1 overflow-hidden flex bg-zinc-50/50 p-4 gap-4">
          <div className="w-1/2 bg-white border border-zinc-200 rounded-2xl flex flex-col overflow-hidden shadow-sm">
            <div className="p-3 border-b border-zinc-100">
              <div className="relative">
                ...
                <input type="text" placeholder="Buscar concepto..." value={searchConcepto} onChange={e => setSearchConcepto(e.target.value)} className="w-full pl-8 pr-3 py-1.5 bg-zinc-50 border border-zinc-200 rounded-lg text-xs focus:outline-none focus:border-indigo-500 transition-all" />
              </div>
            </div>
                ...
                return (
                  <div 
                    ...
                    className={`p-2.5 rounded-lg border flex justify-between items-center transition-all ${
                      yaAgregado ? 'bg-zinc-50 border-transparent opacity-40 cursor-not-allowed' : 'bg-white border-zinc-200 cursor-pointer hover:border-indigo-300 hover:shadow-sm group'
                    }`}
                  >
                     ...
                     {!yaAgregado && <Plus className="w-4 h-4 text-zinc-300 group-hover:text-indigo-500 shrink-0 transition-colors" />}
                  </div>
                );
              })}
            </div>
          ...
          <div className="w-1/2 bg-white border border-zinc-200 rounded-2xl flex flex-col overflow-hidden shadow-sm">
            <div className="p-3 border-b border-zinc-100 bg-zinc-50">
              <h4 className="font-black text-xs text-zinc-800 uppercase tracking-widest">Conceptos Asociados</h4>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-2 space-y-1.5">
              {conceptosRelacionados.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-zinc-400">
                  ...
                  <p className="text-xs font-medium text-center px-4">Haz clic en los conceptos de la izquierda para agregarlos a la liquidación.</p>
                </div>
              ) : (
                conceptosRelacionados.map((item, index) => (
                  <div key={item.conceptoId} className="bg-white border border-zinc-200 rounded-lg p-2 flex items-center gap-2 shadow-sm hover:border-indigo-300 transition-colors">
                    <div className="flex items-center bg-zinc-100 rounded-md p-0.5 shrink-0 border border-zinc-200">
                      <button onClick={() => moverOrden(index, 'UP')} disabled={index === 0} className="p-1 text-zinc-500 hover:text-indigo-600 hover:bg-white rounded disabled:opacity-30 disabled:bg-transparent"><ArrowUp className="w-3 h-3"/></button>
                      <span className="px-1.5 text-[10px] font-black text-zinc-700 w-4 text-center">{item.ordenCalculo}</span>
                      <button onClick={() => moverOrden(index, 'DOWN')} disabled={index === conceptosRelacionados.length - 1} className="p-1 text-zinc-500 hover:text-indigo-600 hover:bg-white rounded disabled:opacity-30 disabled:bg-transparent"><ArrowDown className="w-3 h-3"/></button>
                    </div>
                    <div className="flex-1 min-w-0">
                      <h5 className="font-bold text-zinc-900 text-xs truncate">{item.conceptoNombre}</h5>
                      <p className="text-[9px] font-mono text-indigo-500 font-bold">{item.codigo}</p>
                    </div>
                    <button onClick={() => quitarConcepto(item.conceptoId)} className="w-7 h-7 rounded-md bg-red-50 text-red-500 flex items-center justify-center hover:bg-red-500 hover:text-white transition-colors shrink-0">
        ...
        <div className="p-4 border-t border-zinc-100 bg-white shrink-0">
          <div className="flex gap-3 justify-end">
            <button onClick={() => setIsBuilderOpen(false)} className="px-5 py-2 rounded-xl border border-zinc-200 text-zinc-600 font-bold hover:bg-zinc-50 transition-colors text-sm">Cancelar</button>
            <button onClick={guardarRelacion} className="px-6 py-2 rounded-xl bg-black text-white font-bold hover:bg-zinc-800 shadow-md transition-all text-sm flex items-center gap-2">
              <CheckCircle2 className="w-4 h-4" /> Guardar Relaciones
            </button>
          </div>
        </div>
      </div>
      ...
      <ConfirmDialog 
        ...
        title="Eliminar Liquidación"
        message={`¿Estás seguro que deseas eliminar "${confirmDialog.nombre}"? Se perderá la configuración de los conceptos asociados.`}
